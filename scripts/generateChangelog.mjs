...
async function main() {
  ...
  for (const sub of submodules) {
    ...
    const subBefore = run(`git ls-tree ${beforeSha} ${sub} | awk '{print $3}'`);
    const subAfter = run(`git ls-tree ${afterSha} ${sub} | awk '{print $3}'`);
      ...
      if (sub === "apps/server") {
        ...
        for (const { dir, title } of subSections) {
          ...
          try {
            version = JSON.parse(
              fs.readFileSync(
                path.resolve(process.cwd(), sub, "src", dir, "version.json"),
                "utf-8",
              ),
            ).version;
          } catch {
